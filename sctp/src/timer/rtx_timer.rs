...
impl<T: 'static + RtxTimerObserver + Send> RtxTimer<T> {
    ...
    pub(crate) async fn start(&self, rto: u64) -> bool {
        self.start_after(rto, rto).await
    }

    /// Starts the timer to expire after `first`, and to back off from `rto` from then on:
    /// a timer restarted part-way through an RTO still doubles the RTO, not the remainder.
    pub(crate) async fn start_after(&self, first: u64, rto: u64) -> bool {
        ...
        tokio::spawn(async move {
            ...
            loop {
                let interval = if n_rtos == 0 {
                    first
                } else {
                    calculate_next_timeout(rto, n_rtos)
                };
